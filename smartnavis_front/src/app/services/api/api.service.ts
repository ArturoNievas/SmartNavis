...
export class ApiService {
  ...
  private handleError(error: HttpErrorResponse, method: string, endpoint: string, payload?: any): Observable<never> {
  ...
  public put<T>(endpoint: string, payload: T): Observable<T> {
    return this.http.put<T>(this.baseUrl + endpoint, payload, {})
      .pipe(
        catchError((error) => this.handleError(error, 'PUT', endpoint, payload))
      );
  }
