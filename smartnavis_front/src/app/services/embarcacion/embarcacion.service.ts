...
export class EmbarcacionService {
  ...
  public actualizarEmbarcacion(embarcacion: Embarcacion): Observable<Embarcacion> {
    if (!embarcacion.matricula) {
      return throwError(() => new Error('No es posible actualizar la embarcacion (no matricula).'));
    }
    return this.apiService.put<Embarcacion>(`${this.embarcacionUrl}/${embarcacion.matricula}`, embarcacion);
  }

  public eliminarEmbarcacion(embarcacion: Embarcacion): Observable<unknown> {
    if (!embarcacion.matricula) {
      return throwError(() => new Error('No es posible eliminar la embarcacion (no matricula).'));
    }
